...
function useFetch<TData = any>(
  ...
  optionsNoURLs?: Omit<Options, "url">
): UseFetch<TData> {
  const context = useContext(FetchContext);

  invariant(
    !!urlOrOptions || !!context.url,
  ...
  const handleUseFetchOptions = useCallback(
    (useFetchOptions?: OptionsMaybeURL): void => {
  ...
  if (isString(urlOrOptions) && isObject(optionsNoURLs)) {
    ...
    invariant(
      !optionsNoURLs,
      "You cannot have a 2nd parameter of useFetch when your first argument is a object config."
    );
    const optsWithURL = urlOrOptions as Options;
    invariant(
      !!context.url || !!optsWithURL.url,
      'You have to either set a URL in your options config or set a global URL in your <Provider url="https://url.com"></Provider>'
    );
  ...
  const makeFetch = useCallback(
    (method: HTTPMethod) => {
      ...
      async function doFetch(
        ...
        if (method !== HTTPMethod.GET) {
          ...
          invariant(
            !isObject(routeOrBody),
            "Cannot pass a request body in a GET request"
          );
    ...
    [context.options, data, options, url]
  );
  ...
  const query = useCallback(
    (query: string, variables?: object) => post({ query, variables }),
    [post]
  );
  const mutate = useCallback(
    (mutation: string, variables?: object) => post({ mutation, variables }),
    [post]
  );
  ...
  const request: FetchCommands = useMemo(
    () => ({ get, post, patch, put, del, delete: del, abort, query, mutate }),
    [abort, del, get, mutate, patch, post, put, query]
  );
  ...
  useEffect(() => {
    const methodName = (
      ...
      "" ||
      HTTPMethod.GET
    ).toUpperCase() as keyof FetchCommands;
  ...
  }, [onMount, options.body, options.method, request, url]);
