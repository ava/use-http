...
describe('usePost - BROWSER - general usage', (): void => {
  ...
  it('should execute POST command with object destructuring', async (): Promise<void> => {
  ...
  it('should work with body in requestInit and onMount: true', async (): Promise<void> => {
    const { result, waitForNextUpdate } = renderHook(() => usePost('https://example.com', {
      body: data,
      onMount: true
    }))
  ...
  it('should work with Provider URL and no URL as 1st arg with onMount: true', async (): Promise<void> => {
  ...
  it('no URL as 1st arg - <Provider url={url} /> - request.post()', async (): Promise<void> => {
    const wrapper = ({ children }: { children?: ReactNode }): ReactElement => (
      <Provider url='https://example.com'>{children as ReactElement}</Provider>
    )
    const { result, waitForNextUpdate } = renderHook(() => usePost({
