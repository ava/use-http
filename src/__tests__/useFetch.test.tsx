...
describe('useFetch - BROWSER - basic functionality', (): void => {
  ...
  const wrapper = ({ children }: { children?: ReactNode }): ReactElement => (
    <Provider url="https://example.com" options={{ cachePolicy: NO_CACHE }}>
      {children}
    </Provider>
  )
  ...
  > => {
    ...
    const [request, , loading, error] = result.current
    expect(request.loading).toBe(true)
    expect(loading).toBe(true)
    expect(error).toBe(undefined)
    await waitForNextUpdate()
    const [secRequest, secResponse, secLoading] = result.current
    expect(secResponse.data).toEqual(expected)
    expect(secRequest.loading).toBe(false)
    expect(secLoading).toBe(false)
  })
})
...
describe('timeouts', (): void => {
  ...
  it('should execute GET and timeout after 1000ms, and fire `onTimeout` and `onAbort`', async (): Promise<
    void
  > => {
...
describe('caching - useFetch - BROWSER', (): void => {
  ...
  it('should not make a second request to the same url + route for `cache-first` cachePolicy', async (): Promise<
    void
  > => {
    // run the request on mount
    const { result, waitForNextUpdate } = renderHook(() =>
      useFetch('https://example.com/todos', []),
    )
    expect(result.current.loading).toBe(true)
    await waitForNextUpdate()
    expect(result.current.loading).toBe(false)
...
describe('useFetch - BROWSER - interceptors', (): void => {
  ...
  it('should pass the proper path string to `interceptors.request`', async (): Promise<
    void
  > => {
    const { result } = renderHook(() => useFetch({ path: '/path' }), {
      wrapper,
    })
    await result.current.get()
  ...
  it('should pass the proper url string to `interceptors.request`', async (): Promise<
    void
  > => {
    const { result } = renderHook(() => useFetch('url'), { wrapper })
    await result.current.get()
...
describe('useFetch - BROWSER - errors', (): void => {
  ...
  it('should set the `error` object when response.ok is false', async (): Promise<
    void
  > => {
    fetch.resetMocks()
  ...
  it('should leave the default `data` as array if response is undefined or error', async (): Promise<
    void
  > => {
    const { result } = renderHook(() =>
      useFetch({
  ...
  it('should set the `error` properly for `interceptors.response`', async (): Promise<
    void
  > => {
    const { result } = renderHook(() => useFetch(), {
  ...
  it('should set the `error` properly for `interceptors.response` onMount', async (): Promise<
    void
  > => {
    const { result, waitForNextUpdate } = renderHook(
      () => useFetch('https://example.com', []), // onMount === true
