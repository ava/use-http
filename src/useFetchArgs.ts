...
const defaults = Object.entries(useFetchArgsDefaults).reduce(
  (acc, [key, value]) => {
...
export default function useFetchArgs(
  ...
  optionsNoURLsOrOverwriteGlobalOrDeps?:
    | NoUrlOptions
    | OverwriteGlobalOptions
    | any[],
  deps?: any[],
  ...
  context.options = useMemo(() => {
    const overwriteGlobalOptions = (isFunction(urlOrOptionsOrOverwriteGlobal)
      ? urlOrOptionsOrOverwriteGlobal
      : isFunction(optionsNoURLsOrOverwriteGlobalOrDeps) &&
        optionsNoURLsOrOverwriteGlobalOrDeps) as OverwriteGlobalOptions
  ...
  const onTimeout = useField<() => void>(
    'onTimeout',
    urlOrOptions,
    optionsNoURLs,
  )
  const onNewData = useField<() => void>(
    'onNewData',
    urlOrOptions,
    optionsNoURLs,
  )
  const perPage = useField<number>('perPage', urlOrOptions, optionsNoURLs)
  const cachePolicy = useField<CachePolicies>(
    'cachePolicy',
    urlOrOptions,
    optionsNoURLs,
  )
  ...
  const interceptors = useMemo((): Interceptors => {
    const contextInterceptors =
      (context.options && context.options.interceptors) || {}
  ...
  const requestInit = useMemo((): RequestInit => {
    const contextRequestInit = pullOutRequestInit(
      context.options as OptionsMaybeURL,
    )
